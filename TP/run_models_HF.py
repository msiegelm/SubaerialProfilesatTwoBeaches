...
tstart = np.datetime64('2021-10-07 00:00:00') #YMD
tend =   np.datetime64('2022-03-17 21:02:21') #YMD
...
if ntrials == 1:
    ...
    for ii,jf in enumerate(paramroots):
        ...
        ax.set_ylim([-15,20])
        if ii == ntrials-1:
            ax.set_xlabel("Days Since October 11, 2021",fontsize=20)
        if ii == 0:
...
for ii,jm in enumerate(paramroots):
    paramroot = jm
    df = table_skill_full(modeloutput[paramroot],modelnames,paramroots[ii],saveas=True,figpath=figpath,svname="tb_skill_%s_HF.png"%paramroot)

eqskill = model_skill_statistics(sm["xmhw_anom"],Sogi)
table_skill_full_eq(eqskill,"Eq","Equilibrium",saveas=True,figpath=figpath,svname="tb_skill_eq_HF.png")
# #
# paramroot = "9030Std"
...
# ax.plot(sm["xmhw_anom"],pYdt,".",color="slateblue",markersize=7,label="DT")
# ax.plot(sm["xmhw_anom"],pYrf,".",markersize=7,color="orchid",label="RF")
ax.plot(sm["xmhw_anom"],modeloutput[jf]["ET"]["pY"],".",markersize=7,color="tab:pink",label="ET")
# ax.plot(sm["xmhw_anom"],pYet,".",color="magenta",markersize=7,label="ET")
ax.tick_params(labelsize=16)
ax.set_ylim([-10,20])
ax.set_xlim([-10,20])
