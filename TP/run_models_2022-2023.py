...
from tools import *
sys.path.append('/Users/miks/Desktop/MOP/TorreyPines/ML_revisions/tools/data')
from sm_files_functions import *
...
##################################################
data2 = pd.read_csv(fpath + "MOP%s_MLfiles_ver_eof_2022-2023.csv" %mopnum)
t1 = dates.date2num(np.datetime64("2022-03-17"))
...
tstart = np.datetime64('2022-03-17 21:02:21') #YMD
tend =   np.datetime64('2024-01-01 21:02:21') #YMD
...
plot_ts(dates.num2date(dataV["tnum"]),dataV[features_cols],saveas=True,figpath=figpath,svname="pl_timeseries_LTMOP581_test.png")
...
t1 = dates.date2num(np.datetime64("2022-03-17"))
t2 = dates.date2num(np.datetime64("2023-01-01"))

kk = (dataV["tnum"] > t1) & (dataV["tnum"] < t2)

model_skill_statistics(sm["xmhw_anom"][kk],modeloutput["EOFMeanStdHF"]["ET"]["pY"][kk])

model_skill_statistics(sm["xmhw_anom"][kk],Sogi2015[kk])

t1 = dates.date2num(np.datetime64("2023-01-01"))
t2 = dates.date2num(np.datetime64("2024-01-01"))
...
if ntrials == 1:
    ...
    for ii,jf in enumerate(paramroots):
        ax=axs[ii]
        ax.plot(dates.num2date(sm["tnum"]),modeloutput[jf]["SVR"]["pY"],"*-",label="SVR",color="cyan",markersize=3)
        ax.plot(dates.num2date(sm["tnum"]),modeloutput[jf]["LR"]["pY"],"*-",label="LR",color="orange",markersize=3)
        ax.plot(dates.num2date(sm["tnum"]),modeloutput[jf]["Lasso"]["pY"],"D-",label="Lasso",color="green",markersize=3)
        ax.plot(dates.num2date(sm["tnum"]),modeloutput[jf]["DT"]["pY"],"P-",label="DT",color="gold",markersize=3)
        ax.plot(dates.num2date(sm["tnum"]),modeloutput[jf]["GBR"]["pY"],"v-",label="GBR",color="red",markersize=3)
        ax.plot(dates.num2date(sm["tnum"]),modeloutput[jf]["ET"]["pY"],"s-",color="tab:blue",label="ET",markersize=3)
        ax.plot(dates.num2date(sm["tnum"]),sm["xmhw_anom"],".-k",label="Observations",markersize=5,zorder=1000)
        # ax.plot(sm["tnum"]-sm["tnum"][0],Sogi,"--",label="Equilibrium Model",color="tab:orange",markersize=3)
        ax.set_title(paramroots_titles[ii],fontsize=16)
        ax.tick_params(labelsize=16)
        ax.set_ylabel("$X_{MHW}$ Anomaly [m]", fontsize=16)
        ...
        if ii == 0:
            ...
            fig.suptitle("Torrey Pines (MOP 581)",fontsize=20,y=.94)
...
for ii,jm in enumerate(paramroots):
    paramroot = jm
    df = table_skill_full(modeloutput[paramroot],modelnames,paramroots[ii],saveas=False,figpath=figpath,svname="tb_skill_%s.png"%paramroot)

eqskill = model_skill_statistics(sm["xmhw_anom"],Sogi)
table_skill_full_eq(eqskill,"Eq","Equilibrium",saveas=False,figpath=figpath,svname="tb_skill_eq.png")
# #
# paramroot = "9030Std"
...
fig.savefig(figpath + "pl_MLETVEQ_TP_stats"  + ".png",dpi=300,bbox_inches="tight",pad_inches=.5)
##################################################################################################################
fig,ax=plt.subplots(figsize=(10,4))
ax.plot(sm["tnum"]-sm["tnum"][0],sm["xmhw_anom"],".-k",label="Observations")
ax.plot(sm["tnum"]-sm["tnum"][0],SogOGi,".--",label="Eq (Y09)",color="tab:orange")
ax.plot(sm["tnum"]-sm["tnum"][0],Sogi,">--",label="Eq (Optimized)",color="tab:orange",markersize=4)
ax.plot(sm["tnum"]-sm["tnum"][0],modeloutput[jf][modname]["pY"],".-",label="ET",color="tab:blue")
...
fig.savefig(figpath + "pl_MLETVEQ_TP_nostats"  + ".png",dpi=300,bbox_inches="tight",pad_inches=.5)
####################################################################################################################
f = paramroots[2]
