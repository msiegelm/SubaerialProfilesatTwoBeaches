...
# fVpath = "/Users/miks/Desktop/MOP/TorreyPines/ML_revisions/test_ML/features/csv/"
...
tstart = np.datetime64('2014-10-08T23:00:00') #YMD
...
if ntrials == 1:
    ...
    for ii,jf in enumerate(paramroots):
        ...
        # ax.plot(sm["t64"],Sogi,"--",label="Equilibrium Model",color="grey",markersize=3)
        ax.set_title(paramroots_titles[ii],fontsize=16)
        ax.tick_params(labelsize=16)
        ax.set_ylabel("$X_{MHW}$ Anomaly [m]", fontsize=16)
        ax.set_xlim([sm["t64"][0],sm["t64"][-1]])
        ...
        ax.set_ylim([-45,15])
        ax.tick_params("x",rotation=45)
        fig.suptitle("Black's North (MOP 551)",fontsize=24,y=.95)
...
fig.savefig(figpath + "pl_scatter_methods_HFnoeq.png",dpi=300)
...
ax.plot(sm["t64"],Sogi,">--",label="Eq (Optimized) ($r^2$= %.03f, RMSE = %.03f)"%(req**2,rmseeq),color="tab:red",markersize=4)
# ax.plot(ya["t"],ya["Sog"],".-",label="Equilibrium Model ($r^2$= %.03f, RMSE = %.03f)"%(req**2,rmseeq),color="tab:orange")
ax.plot(sm["t64"],modeloutput[jf][modname]["pY"],".-",label="ML Model (ET) ($r^2$= %.03f, RMSE = %.03f)"%(rml**2,rmseml),color="tab:pink")


ax.tick_params(labelsize=16)
ax.set_ylabel("$X_{MHW}$ Anomaly [m]",fontsize=16)
ax.legend(loc="lower right",fontsize=12)
ax.set_title("MOP 551",fontsize=16)
# ax.set_xlim([-1,160])
...
fig.savefig(figpath + "pl_MLETVEQ_BN_statsNINO"  + ".png",dpi=300,bbox_inches="tight",pad_inches=.5)
##################################################################################################################
fig,ax=plt.subplots(figsize=(10,4))
ax.plot(sm["t64"],sm["xmhw_anom"],".-k",label="Observations")
...
fig.savefig(figpath + "pl_MLETVEQ_BN_nostatsNINO"  + ".png",dpi=300,bbox_inches="tight",pad_inches=.5)
